...
def test_get_assignments_includes_attachment_metadata(test_client, make_admin):
    make_admin(email="teacher3@example.com", password="teacher", name="teacheruser3")

    test_client.post(
        "/auth/login",
        data=json.dumps({"email": "teacher3@example.com", "password": "teacher"}),
        headers={"Content-Type": "application/json"},
    )

    class_response = test_client.post(
        "/class/create_class",
        data=json.dumps({"name": "CompSci 103"}),
        headers={"Content-Type": "application/json"},
    )
    class_id = class_response.json["class"]["id"]
    ...
    create_response = test_client.post(
        ...
        data={
            ...
            "attachments": (io.BytesIO(b"summary"), "summary.txt"),
        },
        content_type="multipart/form-data",
    )
    ...
    match = next((a for a in list_response.json if a["id"] == assignment_id), None)
    assert match is not None
    assert match["attachments"]
    summary_attachment = next(
        (a for a in match["attachments"] if a["original_name"] == "summary.txt"),
        None,
    )
    assert summary_attachment is not None
    assert summary_attachment["download_url"].startswith(
