...
def test_teacher_can_auto_assign_students_evenly_when_possible(
    test_client, make_admin, enroll_user_in_course
):
    teacher_email = "teacher-auto-even@example.com"
    make_admin(email=teacher_email, password="teacher", name="teacher-auto-even")

    class_id, assignment_id = _create_class_and_assignment(test_client, teacher_email, "teacher")

    student_ids = []
    for index in range(1, 7):
        email = f"student-auto-even-{index}@example.com"
        _register_student(test_client, f"Student Auto Even {index}", email)
        with test_client.application.app_context():
    ...
    auto_assign_response = test_client.post(
        f"/assignment/{assignment_id}/groups/auto-assign",
        headers={"Content-Type": "application/json"},
    )
    assert auto_assign_response.status_code == 200

    group_sizes = sorted(len(group["members"]) for group in auto_assign_response.json["groups"])
    assert group_sizes == [2, 2, 2]

    assigned_student_ids = sorted(
        student["id"] for student in auto_assign_response.json["students"] if student.get("groupID")
    )
    assert assigned_student_ids == sorted(student_ids)


def test_teacher_auto_assign_keeps_group_sizes_even_when_possible_with_odd_total(
    test_client, make_admin, enroll_user_in_course
):
    teacher_email = "teacher-auto-odd@example.com"
    make_admin(email=teacher_email, password="teacher", name="teacher-auto-odd")

    class_id, assignment_id = _create_class_and_assignment(test_client, teacher_email, "teacher")

    for index in range(1, 8):
        email = f"student-auto-odd-{index}@example.com"
        _register_student(test_client, f"Student Auto Odd {index}", email)
        with test_client.application.app_context():
            student = User.get_by_email(email)
            enroll_user_in_course(student.id, class_id)

    for name in ["Team One", "Team Two", "Team Three"]:
        response = test_client.post(
            f"/assignment/{assignment_id}/groups",
            data=json.dumps({"name": name}),
            headers={"Content-Type": "application/json"},
    ...
    group_sizes = [len(group["members"]) for group in auto_assign_response.json["groups"]]
    odd_group_count = sum(1 for size in group_sizes if size % 2 != 0)

    assert sum(group_sizes) == 7
    assert odd_group_count == 1
